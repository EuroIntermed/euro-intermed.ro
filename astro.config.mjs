...
const CANON_RO = (process.env.PUBLIC_CANONICAL_RO || 'https://www.euro-intermed.ro').replace(/\/+$/, '')
const CANON_EN = (process.env.PUBLIC_CANONICAL_EN || 'https://www.euro-intermed.com').replace(/\/+$/, '')
...
export default defineConfig({
  ...
  build: {
    // Emit /contact/index.html so the current `cleanUrls` Vercel behaviour keeps
    // working after the migration off the hand-rolled build.mjs.
    format: 'directory',
  },
})
